...
*/
    ...
    if ("DaysFrmTodayChg".equals(pageContext.getParameter(EVENT_PARAM))) {
		...
		if (addDaysFilter != "0") 
		{
			Serializable[] parms1 = {queryID, daysFilter, addDaysFilter};
			retVal = am.invokeMethod("reloadAddDays", parms1).toString();         
			if ("1".equals(retVal)) {
				reload = 1;
				backup = "2";
			}
...
//              SkidQtyTot.setAttributeValue(TABULAR_FUNCTION_VALUE_ATTR,"10");
     ...
     if (pageContext.isLovEvent()) {
        ...
        if ("ContainerLOV".equals(lovID)) {
            OAFormValueBean hQueryID = (OAFormValueBean)webBean.findChildRecursive("ParentQryID");
            String queryID = (String)hQueryID.getValue(pageContext);
			Hashtable lovResults = pageContext.getLovResultsFromSession(lovID);
			String userLov =(String)lovResults.get("ContainerLOV");
            Serializable[] parms = {queryID, userLov};
//		    am.invokeMethod("calcTotals", parms);                  
		    am.invokeMethod("calcTotalsfrm", parms);                  
			
//			throw new OAException("checking lov sathu queryid: " + userLov , OAException.WARNING);   
//            am.invokeMethod("calcTotals", parms);                  
             // Recalculate Totals after Factor Adjustment
             // setColTotals(pageContext, webBean);
             //         throw new OAException(msg, OAException.INFORMATION);
...
//     }        
	 ...
	 if ("chgContainerLOVt".equals(pageContext.getParameter(EVENT_PARAM))) {
...
//            columnBean.setAttributeValue(ON_SUBMIT_VALIDATER_ATTR, formatter);
    ...
    public void setColTotals(OAPageContext pageContext, OAWebBean webBean) {
        ...
        {  
            ...
            Query = Query + "to_char(nvl(sum(sum_addqty), 0), '999,999,990') sum_addqty, ";
            ...
            Query = Query + "nvl(sum(qty_bkt50), 0) sum_bkt50 ";
            Query = Query + "from xxmb_wkbqry_mtx where nvl(selected,'Y') = 'Y'";  
...
//                String colValue = String.valueOf(resultset.getInt("sum_qty"));
                ...
                colValue = resultset.getString("sum_addqty");
                OAMessageTextInputBean ItemAddQtyTot = (OAMessageTextInputBean) advtableBean.findChildRecursive("AddDaysQty");
                ItemAddQtyTot.setAttributeValue(TABULAR_FUNCTION_VALUE_ATTR, colValue);
